...
Base.prototype.resize = function(fn){
        for(var i = 0; i<this.elements.length; i++){
            ...
            /*
            window.onresize = function(){//¸ü¸Ä´°¿Ú´óÐ¡ ±£Ö¤µ¯´°ÔÚÒ³ÃæÉÏ
                fn();
...
//拖拽功能
Base.prototype.drag = function(fn){
       for(var i = 0; i<this.elements.length; i++){
            addEvent(this.elements[i],'mousedown',function(e){
                  //alert(trim(this.innerHTML).length)
                  if(trim(this.innerHTML).length == 0){//当内容为空的时候，再执行阻止默认行为
                     //e.preventDefault();//阻止了默认行为，因为IE中拖拽空白div会出现bug，但是阻止默认行为之后，输入也被阻止掉了
                  };
                  ...
                  function move(e){
                     ...
                     if(typeof _this.setCapture != 'undefined'){//½â¾öµÍ°æ±¾IE³ö¿òbug
                          _this.setCapture();
                     };
                  }
