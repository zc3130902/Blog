...
Base.prototype.animate = function(obj){
      for(var i = 0; i<this.elements.length; i++){ 
          ...
          var time = obj['time'] != undefined ? obj['time'] : 30;  //可选 默认50毫秒执行一次
          var step = obj['step'] != undefined ? obj['step'] : 10;  //可选 步长
          ...
          clearInterval(window.timer); //定时器如果是按钮触发会累加，导致越来越快 所以每次点击要清空定时器
          timer = setInterval(function(){

              if(type == 'buffer'){
                  step = (target - getStyle(element,attr)) / speed; //（终点-起点）/速度
